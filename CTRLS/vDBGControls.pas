...
type
  ...
  public
    function AddLine(const aCaption: string;
      const aAdditionalCaption: string = ''): TValuesList; // обавить строчку
    function AddValue(const aValue: string): TValuesList; // добавить значени€ в последнюю строчку
    ...
    procedure Commit; // применить изменени€
...
//    property DrawingStyle;
  ...
  // сделать объект дл€ рисовани€ надписей точечным шрифтом
  // грд дл€ отображени€ регистров и флагов
  ...
  // замутить грид дл€ отображени€ стека
  // сделать, чтобы значени€ брал напр€мую с массива
  ...
  // вспомогательный класс - коллекци€, хран€ща€ на себе дизассемблированный сегмент кода
  PDisasmItem = ^TDisasmItem;
  TDisasmItem = packed record
    Address: Cardinal; // адрес
    Segment: Cardinal; // адрес сегмента кода
    Size: Byte; // кол-во байт в команде
    Opcode: string[16]; // опкод hex строкой
    Disasm: string[32]; // дизасм опкода
  end;
  ...
  protected
    ...
    procedure RowMoved(FromIndex, ToIndex: Longint); override;
  public
    procedure UpdateDisasm; // передизасмить сегмент
...
begin
  ...
  begin
    ...
    case ACol of
      ...
      2: {дизасм}
        begin
          // дизасм можем показывать только наход€сь в сегменте кода
...
var
  ...
  _item: TDisasmItem;
begin
  // парсим только сегмент FSegmentOffset..FSegmentOffset+FSegmentOffsetSize
  Clear;
  // FSegmentOffset - смещение относительно начала блока куска пам€ти (CS)
  // адрес (логический) текущей дизассемблируемой команды == FSegmentOffset + i
  i := 0;
