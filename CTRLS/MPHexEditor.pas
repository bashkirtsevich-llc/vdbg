(*
  ...
  - Dieter Kцhler for reporting the delphi vcl related CanFocus bug<br><br>
  ...
  - Magnus Flysjц for his Delphi2006 package and the updated MPDELVER.INC file<br><br>
...
{$ENDIF}
    procedure Attach(const Memory: Pointer; const Size: Longint);
...
{$ENDIF}
    ...
    *)
    ...
    function PrepareFindReplaceData(StrData: string; const IgnoreCase, IsText:
      boolean): string;
    // attach to allocated memory
    procedure AttachMemory(const Memory: Pointer; const Size: Longint);
...
function OEM2Char(aByte: Byte): Char; inline;
var
  src: AnsiChar;
  dst: PChar;
...
begin
  src := AnsiChar(aByte);
  dst := GetMemory(SizeOf(Char));
  OemToChar(@src, dst);
  Result := Char(@dst[0]);
  ZeroMemory(dst, SizeOf(Char));
  FreeMemory(dst);
//  LszBuf[0] := char(aByte);
//  LszBuf[1] := #0;
...
function Char2OEM(aByte: Byte): Char; inline;
var
  src: Char;
  dst: PAnsiChar;
//  LszBuf: array[0..1] of char;
begin
  src := Char(aByte);
  dst := GetMemory(SizeOf(AnsiChar));
  CharToOem(@src, dst);
  Result := Char(@dst[0]);
  ZeroMemory(dst, SizeOf(AnsiChar));
  FreeMemory(dst);
//  LszBuf[0] := char(aByte);
//  LszBuf[1] := #0;
...
// Obtient la position dans le fichier а partir de la position du curseur
...
begin
  ...
  try
    ...
    if Result then
    begin
      // ьberprьfen, ob linke maustaste oder shift gedrьckt, sonst selection zurьcksetzen
      if not (IsKeyDown(VK_SHIFT) or IsKeyDown(VK_LBUTTON)) then
        ResetSelection(True);

      // ьberprьfen, ob auЯerhalb der DateiGrцЯe
...
begin
  Result := (FSelPosition <> -1) and (APosition >= FSelStart) and (APosition <= FSelEnd);
end;
...
begin
  ...
  begin
    ...
    tmp := GetCursorAtPos(LIntPos, LBoolInCharField);
    if LBool2ndCol then
      Inc(tmp.X);
    MoveColRow(tmp.x, tmp.y, True, True);
...
procedure TCustomMPHexEditor.AttachMemory(const Memory: Pointer;
  const Size: Longint);
begin
  try
//    FreeStorage;
    ...
    FDataStorage.Attach(Memory, Size);
  except
//    FreeStorage;
    FDataStorage.Attach(nil, 0);
    FreeStorage(True);
  end;
end;
...
begin
  ...
  begin
    ...
    // zuerst aktuelle auswahl lцschen
...
begin
  ...
  if tmp.X < GRID_FIXED then
    tmp.X := GRID_FIXED;
  if tmp.Y >= RowCount then
    tmp.Y := RowCount - 1;
  if tmp.Y < GRID_FIXED then
    tmp.Y := GRID_FIXED;
  LIntDragPos := GetPosAtCursor(tmp.X, tmp.Y);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // отрисовка выделенного офрагмента
        ...
        begin
          if (LIntCurCol < LIntLastCol) and (LIntCurCol <> FIntLastHexCol)
            ...
            LColBackColor := clMaroon; //Invert(LColBackColor);
          LColTextBackColor := clMaroon;//Invert(LColTextBackColor);
          LColTextColor := LColTextColor;//Invert(LColTextColor);
      ...
      begin
        ...
        begin
          ...
          if FUnicodeCharacters then
            while (Canvas.Font.Size > 1) and GetTextExtentPoint32W(Canvas.Handle,
              @LWChrOutput, 1, LrecSize) and (LRecSize.cx > (LRect2.Right - LRect2.Left)) do
              Canvas.Font.Size := Canvas.Font.Size -1;
          ExtTextOutW(Handle, Left, Top,
            ETO_CLIPPED or ETO_OPAQUE, @LRect2, @LWChrOutput, 1, nil);
          if FUnicodeCharacters then
...
{$ENDIF}
  ...
  with DrawInfo do
  begin
    Canvas.Brush.Color := TestColor(FColors.Background);
...
begin
  try
    FReplaceUnprintableCharsBy := Char(Byte(Reader.ReadInteger));
  except
    FReplaceUnprintableCharsBy := '.';
  end;
end;
...
begin
  // скролирование вниз
  // доработать, чтоб с зажатым Ctrl скролировалось на страницу
  if Shift <> [] then
  ...
  begin
    // scroll down one page
    TopRow := Min(Max(GRID_FIXED, RowCount - VisibleRowCount),
      TopRow + {VisibleRowCount -} 1); // скролировало на страницу
    CheckSetCaret;
    Result := True;
  end;
end;
...
begin
  ...
  begin
    // scroll up one page
    TopRow := Max(GRID_FIXED, TopRow - {VisibleRowCount +} 1); // скролировало на страницу
    CheckSetCaret;
    Result := True;
  end;
end;
...
function TMPHUndoStorage.Undo: boolean;
  ...
  var
    ...
    tmp: TGridCoord;
  begin
    tmp := FEditor.GetCursorAtPos(aBuffer.CurPos, ufFlagInCharField in aBuffer.Flags);
    if not (ufFlagInCharField in aBuffer.Flags) then
      if FEditor.DataSize > 0 then
        if ufFlag2ndByteCol in aBuffer.Flags then
          tmp.x := tmp.x + 1;

    FEditor.MoveColRow(tmp.x, tmp.y, True, True);
...
function TMPHUndoStorage.Redo: boolean;
  ...
  begin
    ...
    begin
      ...
      tmp := FEditor.GetCursorAtPos(CurPos, ufFlagInCharField in Flags);
...
procedure TMPHMemoryStream.Attach(const Memory: Pointer; const Size: Longint);
begin
