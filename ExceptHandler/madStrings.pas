...
var lowCharTable : array [0..255] of ansichar =
...
function LowChar(const c: char) : char;
begin
  result := Char(lowCharTable[byte(c)]);
end;
...
begin
  ...
  if result then
    for c2 := 1 to c1 do
      if lowCharTable[Byte(s1[c2])] <> lowCharTable[Byte(s2[c2])] then begin
        result := false;
        break;
      end;
end;
...
begin
  ...
  for c4 := 1 to c3 do begin
    ch1 := Char(lowCharTable[Byte(s1[c4])]);
    ch2 := Char(lowCharTable[Byte(s2[c4])]);
...
begin
  ...
  if (subStr <> nil) and ((subStrLen <> 0) or (subStr^ <> #0)) and
    ...
    if strLen >= subStrLen then begin
      c1 := strLen - subStrLen;
      if ignoreCase then ch1 := Char(lowCharTable[Byte(subStr^)])
      else               ch1 :=              subStr^;
      if fromPos > toPos then begin
        if toPos <= c1 then begin
          ...
          if ignoreCase then begin
            while pc1 >= pc2 do
              if Char(lowCharTable[Byte(pc1^)]) = ch1 then begin
                inc(pc1);
                pc5 := pc1;
                while (pc3 <= pc4) and (lowCharTable[Byte(pc1^)] = lowCharTable[Byte(pc3^)]) do begin
                  inc(pc1); inc(pc3);
                end;
  ...
  if not result then begin
    ...
    while (cm^ <> #0) or (cs^ <> #0) do begin
      ...
      case cm^ of
        ...
        else if lowCharTable[Byte(cm^)] <> lowCharTable[Byte(cs^)] then begin
               if b1 then begin
                 cm := mm;
                 cs := ms;
                 inc(ms);
  ...
  while true do begin
    ...
    case cm^ of
      ...
      ',',CSpecialCharEnd : if cm - pc1 = 1 then begin  // ",x," single char, must be 1 byte long
                              if ignoreCase then
                                   Include(cset, AnsiChar(lowCharTable[Byte(pc1^)]))      // fill char set
                              else Include(cset, AnsiChar(pc1^));
      ...
      '.'                 : if cm - pc1 = 1 then begin  // ",x..y," multiple chars, both x and y must be 1 byte long
                              ...
                              for ch1 := pc1^ to cm^ do                   // fill char set
                                if ignoreCase then
                                     Include(cset, AnsiChar(lowCharTable[Byte(ch1)]))
                                else Include(cset, AnsiChar(ch1));
                              inc(cm);
                              if cm^ = CSpecialCharEnd then begin         // we are ready with parsing...
                                inc(cm);
                                break;
      ...
      case cm^ of
        ...
        CSpecialCharBegin:
             begin
               ...
               for i1 := 1 to sc do
                 if (lowCharTable[Byte(cs^)] in cset) = b2 then begin
                   if b1 then begin
                     cm := mm;
                     cs := ms;
                     inc(ms);
...
begin
  if fillLen > 0 then begin
    ...
    //system.FillChar(s1[1], fillLen * 2, fillChar);
...
var valTable : array [0..255] of byte =
...
begin
  ...
  if str <> nil then begin
    b1 := str^ = '-';
    if not b1 then result := valTable[Byte(str^)];
    inc(str);
    if hex then begin
      for i1 := 1 to len - 1 do begin
        result := (result shl 4) + valTable[Byte(str^)];
        inc(str);
      end;
    end else
      for i1 := 1 to len - 1 do begin
        result := (result * 10) + valTable[Byte(str^)];
        inc(str);
      end;
...
begin
  ...
  for i1 := 1 to Length(result) do
    result[i1] := Char(byte(result[i1]) xor $55);
end;
